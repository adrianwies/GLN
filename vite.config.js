  ...
  plugins: [
    {
      ...
      configureServer(server) {
        server.middlewares.use((req, _res, next) => {
          ...
          if (/^\/catalogo\/marca\/[a-z0-9]+(?:-[a-z0-9]+)*\/?(?:\?.*)?$/.test(req.url || '')) req.url = '/catalogo/index.html'
          next()
        })
      },
      ...
      transformIndexHtml(html) {
        ...
        return html
          ...
          .replace('<!-- HEADER_COMPONENT -->', `${component('src/components/delivery-banner/delivery-banner.html')}\n${component('src/components/header/header.html')}`)
  ...
  build: {
    rollupOptions: {
      input: {
        ...
        preguntasFrecuentes: resolve(import.meta.dirname, 'preguntas-frecuentes/index.html'),
        terminos: resolve(import.meta.dirname, 'terminos-y-condiciones/index.html'),
        reclamaciones: resolve(import.meta.dirname, 'libro-de-reclamaciones/index.html'),
      },
    },
  },
})
